...
export default function AddonSubSection() {
  const [metricData, setMetricData] = useState<MetricItem[]>([
    {
      title: "Available Credits",
      metric: 0,
      icon: <FaCalendarCheck size={20} className="max-sm:size-4" />,
    },
    {
      title: "Subscription Ends on",
      metric: 0,
      icon: <FaCalendar size={20} className="max-sm:size-4" />,
    },
    {
      title: "Last Credit Purchase",
      metric: 0,
      icon: <FaCalendarCheck size={20} className="max-sm:size-4" />,
    },
  ]);
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        const currentGym = profileData.data?.associated_gyms.find(
        ...
        setMetricData([
          {
            title: "Available Credits",
            metric: currentGym?.whatsapp_credits?.remaining_credits || 0,
            icon: <FaCalendarCheck size={20} />,
          },
          {
            title: "Subscription Ends on",
            metric: profileData.data?.subscription_end_date
              ? new Date(
          ...
          {
            title: "Last Credit Purchase",
            metric: currentGym?.whatsapp_credits?.last_purchased
              ? new Date(
                  currentGym.whatsapp_credits.last_purchased.split("T")[0]
                ).toLocaleDateString("en-US", {
                  month: "short",
                  day: "numeric",
                  year: "numeric",
  ...
  return (
    <div className="space-y-6">
          ...
          {metricData.map((metric, index) => (
            <div key={index} className="relative group pointer-events-none">
              <MetricCard
                ...
                icon={metric.icon}
                metricClassName="text-primary text-xs text-start pl-2 "
              />
            </div>
          ))}
        </div>
      ...
      <WidgetCard
        ...
        title="WhatsApp Credit Plans"
        titleClassName="whitespace-nowrap "
      >
        <BillingSettingsView isRegister={false} type="addon" />
      </WidgetCard>
    </div>
  );
}
