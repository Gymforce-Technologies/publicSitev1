...
export default function Attachments({
  ...
  const getAttachments = async (): Promise<void> => {
    try {
      ...
      const resp = await AxiosPrivate.get(
        `/api/staff/${id}/attachments/?gym_id=${gymId}&${currentPage ? `page=${currentPage}` : ""}`,
        {
          id: newID(`staff-attachments-${id}-${currentPage ? currentPage : 1}`),
        }
      );
  ...
  return (
    ...
    >
            ...
            {attachments.map((attachment) => (
              ...
              >
                <div className="flex items-start gap-3 w-full">
                  ...
                  <div className="grid gap-2 w-full">
                    <Text className="font-bold ">
                      {attachment.description.split("--")[1].trim()}
                    </Text>
                    <div className="font-semibold flex flex-row items-end justify-between">
                      <Text>
                        Info : {attachment.description.split("--")[0].trim()}
                      </Text>
                ...
                <Button
                  ...
                  className="flex items-center space-x-1 max-sm:scale-90"
                >
      ...
      <Button
        className="flex flex-row gap-2 max-sm:scale-90 justify-center items-center transition-transform hover:scale-105 duration-200 font-medium mx-auto"
        onClick={() => {
          if (!isValid) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await AxiosPrivate.post(
        `/api/staff/${id}/attachments/?gym_id=${gymId}`,
        formData,
        {
          headers: {
        ...
        {file && !error && (
          <div className="mb-4 grid gap-2 font-medium">
            ...
            {file.type === "application/pdf" && (
              <div className="flex items-center justify-center bg-gray-100 p-4 rounded">
                ...
                <Button
                  ...
                  className="flex flex-row gap-2 justify-center items-center transition-transform hover:scale-105 duration-200 font-medium"
                >
            ...
            <Button
              ...
              className="flex flex-row max-sm:scale-90 justify-center items-center transition-transform hover:scale-105 duration-200 font-medium"
            >
