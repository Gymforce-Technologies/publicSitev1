...
import { FollowUpType } from "./FollowUps";
...
export default function EditFollowUpLead({
  id,
  refreshData,
  setOpenMemberId,
  data,
  ...
  useEffect(() => {
    if (data !== null && trainers && purposes && comments) {
      ...
      const isCustomPurposeCheck = !purposes.find(
        (purpose) =>
          purpose?.value?.toLowerCase() === data?.purpose?.toLowerCase()
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await AxiosPrivate.patch(`/api/followups/${data?.id}/?gym_id=${gymId}`, {
        ...
        lead_id: data?.lead?.id, // Changed from member_id to lead_id
      ...
      toast.error("Something went wrong while updating followup");
    } finally {
