...
const UpdateExpenseModal = ({
  ...
  useEffect(() => {
    if (expense) {
      reset({
        ...
        notes: expense.notes,
        payment_mode_id: expense.payment_mode.toString(),
      });
      setCategoryValue(
        categoryOptions.find((option) => option.value === expense.category) ||
          null
      );
      setPaymentModeValue(expense.payment_mode.toString());
    }
    setErrors({});
  }, [expense, reset]);
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const URL = `/api/update-expense/${expense.id}/?gym_id=${gymId}`;
      await AxiosPrivate.put(URL, formattedData).then(() => {
        invalidateAll();
      });
        ...
        >
          <div className="grid grid-cols-1 gap-2 ">
            ...
            <div className="mb-2 col-span-full">
              ...
              <Controller
                ...
                control={control}
                render={({ field }) => (
                  <Textarea {...field} id="remark" placeholder="Remark" />
                )}
              />
            </div>
          </div>
          <div className="mt-auto pt-4 flex justify-between w-full space-x-1">
            <Button
              ...
              onClick={onClose}
              variant="outline"
              className="text-gray-900  hover:text-primary w-full"
            >
              Cancel
            </Button>
            <Button type="submit" disabled={lock} className="w-full">
