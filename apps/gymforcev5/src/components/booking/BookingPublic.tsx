  ...
  useEffect(() => {
    const getSessions = async () => {
      ...
      const resp = await AxiosPublic.get(
        `https://apiv2.gymforce.in/api/classes/available/v2/?gym_id=${gymId}&end_date=${formateDateValue(selectedDate, "YYYY-MM-DD")}`,
        {
          id: `Gym-${gymId}-${formateDateValue(selectedDate, "YYYY-MM-DD")}`,
        }
      );
