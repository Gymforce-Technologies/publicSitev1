...
export default function PublicGymSection() {
  ...
  const getGallery = async (gymId: string) => {
    const resp = await AxiosPublic.get(
      `https://apiv2.gymforce.in/center/list-gallery/${gymId}/`,
      {
  ...
  const getTrainerDetails = async (gymId: string) => {
    const resp = await AxiosPublic.get(
      `https://apiv2.gymforce.in/center/list-trainer/${gymId}/`,
      {
  ...
  const getOffers = async (gymId: string) => {
    try {
      const resp = await AxiosPublic.get(
        `https://apiv2.gymforce.in/center/list-offers/${gymId}/`,
        {
          id: `Gym-${gymId}-Offers`,
        }
      );
  ...
  const handleEnquirySubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const response = await AxiosPublic.post(
        `https://apiv2.gymforce.in/center/new-enquiry/${gymId}`,
        formData,
        {
          headers: {
